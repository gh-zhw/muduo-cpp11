...
ssize_t Buffer::readFd(int fd, int *saveErrno)
{
    ...
    /*
    struct iovec {
        void *iov_base; // iov_base指向的缓冲区存放的是readv所接收的数据或是writev将要发送的数据
...
// outputBuffer_.writeFd标示将数据写给fd，从readerIndex_开始，可以写readableBytes()个字节
// write() 并不保证写完了全部数据，可能只写了部分数据
// 必须由调用者根据返回值 n 调用 retrieve(n) 来移动 readerIndex_
