...
EpollPoller::EpollPoller(EventLoop *loop)
    : Poller(loop)
    , epollfd_(::epoll_create1(EPOLL_CLOEXEC)) // exec 后自动关闭 fd
...
Timestamp EpollPoller::poll(int timeoutMs, ChannelList *activeChannels)
{
    ...
    int saveErrno = errno;  // errno 可能会被修改，因此立即保存
...
// 从Poller中删除channel(epoll和channels_中都删除)
