 ...
 * 1. 使类内部可以通过 shared_from_this() 安全地获得指向自身的 shared_ptr
 * 2. 防止异步回调或延迟操作中 this 悬空
**/
...
private:
    ...
    // “高水位”在网络编程中指的是发送缓冲区中数据的大小超过了某个阈值；是一个流量控制机制，用于防止写入过快导致内存爆炸或延迟增加
    // 这些回调TcpServer也有 用户通过写入TcpServer注册 TcpServer再将注册的回调传递给TcpConnection TcpConnection再将回调注册到Channel中
    ConnectionCallback connectionCallback_;       // 有新连接或连接状态变化时的回调
    MessageCallback messageCallback_;             // 有读写消息时的回调
    WriteCompleteCallback writeCompleteCallback_; // 消息发送完成以后的回调
    HighWaterMarkCallback highWaterMarkCallback_; // 高水位回调
    CloseCallback closeCallback_; // 关闭连接的回调
    size_t highWaterMark_; // 高水位阈值；当发送缓冲区大小超过 highWaterMark_ 时调用 highWaterMarkCallback_ 提醒用户程序
