...
{
public:
    static const size_t kCheapPrepend = 8;   // 在缓冲区起始位置预留的prependabel空间大小
    static const size_t kInitialSize = 1024; // 实际数据区的初始化大小
...
private:
    ...
    void makeSpace(size_t len)
    {
         ...
         **/
        if (writableBytes() + prependableBytes() < len + kCheapPrepend) // len > xxx + writer
        {
            buffer_.resize(writerIndex_ + len); // writer = len
        }
    ...
    size_t readerIndex_;        // 可读空间的起始位置
    size_t writerIndex_;        // 可写空间的起始位置
};